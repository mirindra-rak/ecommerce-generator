...
import { deleteFacetAction } from "@/app/admin/(protected)/facettes/_actions";
import { Input, PencilIcon, SearchIcon, TrashIcon, cx } from "@pharmacie/ui";
...
export function FacettesTable({ initialData }: { initialData: FacetRow[] }) {
  const t = useTranslations("admin.facets");
  ...
  const columns = useMemo<ColumnDef<FacetRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: t("table.colValues"),
        cell: ({ getValue }) => <span className="text-sm text-muted">{getValue<number>()}</span>,
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center justify-end gap-0.5">
            <Link
              href={`/admin/facettes/${row.original.id}`}
              title={t("table.edit")}
